...
/**
 * cli#parse(commands, dir)
 * - commands(Object): mapping of command name to its options and callback action
 * - dir(String): directory where the client module is located, used as a base directory to read package.json file
 *
...
function parse(commands, dir) {
  ...
  _.each(commands, function (command, name) {
    ...
    _.each(command.options, function (option) {
      program.option(option.arg, option.desc);
    });
