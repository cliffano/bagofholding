...
function fs(checks, mocks) {
  ...
  function exists(file, cb) {
    checks['fs_exists_' + file] = file;
    cb(mocks['fs_exists_' + file] || false);
  }
  ...
  function readFile(file, encoding, cb) {
    checks['fs_readFile_file_' + file] = file;
    checks['fs_readFile_encoding_' + file] = encoding;
    cb(mocks['fs_readFile_error_' + file], mocks['fs_readFile_data_' + file]);
  }
